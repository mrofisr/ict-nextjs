...
export default async function handler(req, res) {
	...
	switch (role) {
		...
		case "user_penitipan":
			...
			const checkPasswordUserPenitipan = bcrypt.compareSync(
				password,
				selectUserPenitipan.password
			);
			if (!checkPasswordUserPenitipan)
				return res.status(404).json({
					...error,
					error_status: 404,
					message: "Wrong old password",
			...
			const passwordHashPentipan = bcrypt.hashSync(
				new_password,
				saltUserPenitipan
			);

			const changePasswordUserPentipan =
				await prisma.table_user_penitipan.update({
					where: { id_user_penitipan: parseInt(id) },
